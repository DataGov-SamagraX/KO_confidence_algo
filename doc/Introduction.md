...
<strong>$s_0$* (f):  Scaled Adjusted Confidence:</strong>  Adjusted confidence of fact that has been scaled logistically (from 0-1). This is the final score that will be used as the confidence level
...
<strong>𝞂*(f):</strong>  Adjusted confidence score for a fact:</strong>  This is a logarithmic transformed version of adjusted confidence s*(f) to prevent underflow
